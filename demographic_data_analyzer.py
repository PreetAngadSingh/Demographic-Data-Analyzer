...
def calculate_demographic_data(print_data=True):
    ...
    # with and without `Bachelors`, `Masters`, or `Doctorate` and percentage with salary >50K
    higher_education_rich = round(((education & earnings).sum()/education.sum())*100,1)
    lower_education_rich = round(((~education & earnings).sum()/(~education).sum())*100,1)
